...
def parsePoint(line):
    ...
    return LabeledPoint(lineBuffer[0],lineBuffer[1:])

parsedSamplePoints = samplePoints
firstPointFeatures = parsePoint(samplePoints[0]).features
firstPointLabel = parsePoint(samplePoints[0]).label
...
minYear = onlyLabels[0]
maxYear = onlyLabels[-1]
...
def shiftLabel(item):
    item.label -= minYear
    return item
parsedData = parsedDataInit.map(shiftLabel)
...
def calcRMSE(labelsAndPreds):
    ...
    """
    #return (labelsAndPreds.map(lambda (label,prediction): squaredError(label,prediction)).mean())**0.5
    return (labelsAndPreds.map(lambda (label,prediction): (label-prediction)**2).mean())**0.5
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTrain = parsedTrainData.map(lambda x:(x.label, averageTrainYear)).cache()
rmseTrainBase = calcRMSE(labelsAndPredsTrain)

labelsAndPredsVal = parsedValData.map(lambda x:(x.label, averageTrainYear)).cache()
rmseValBase = calcRMSE(labelsAndPredsVal)

labelsAndPredsTest = parsedTestData.map(lambda x:(x.label, averageTrainYear)).cache()
...
labelsAndPreds = parsedValData.map(lambda lp: (lp.label,weightsLR0.dot(lp.features)))
...
def twoWayInteractions(lp):
    ...
    """
    lp.features = DenseVector(list(np.array(np.matrix(lp.features).T*(np.matrix(lp.features))).flatten()) + 
    list(np.array(np.matrix(lp.features)).flatten())) 
