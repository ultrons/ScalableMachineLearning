...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    return SparseVector(numOHEFeats,pos)
...
def parsePoint(point):
    ...
    """
    features=[]
    for i,c in enumerate(point.split(',')[1:]):
        features.append((i,c))
    return features
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    for i,c in enumerate(pointList[1:]):
        features.append((i,c))
        
    return LabeledPoint(pointList[0], oneHotEncoding(features, OHEDict, numOHEFeats))
...
print SparseVector(100, sampThreeHundredBuckets)
...
def parseHashPoint(point, numBuckets):
    ...
    return LabeledPoint(pointList[0], SparseVector(numBuckets,hashFunction(numBuckets,features)))
...
#print len(hashTrainData.take(1)[0].features.values)
...
def computeSparsity(data, d, n):
    ...
    """
    return (data
           .map(lambda x: len(x.features.indices)*1.0/len(x.features))
           .mean()
           )
